...
type queuedRender struct {
	op         *DrawOp
	renderFunc func(screen *ebiten.Image, camera *camera.Camera)
	mode       RenderMode
	ordering   int
}
...
func QueueFunc(renderFunc func(screen *ebiten.Image, camera *camera.Camera), layer int) {
	renderQueue = append(renderQueue, &queuedRender{
		op:         nil,
...
// Render will draw all queued operations to the screen.
// It will clear the screen, and clear the queue when it is done.
func Render(screen *ebiten.Image, camera *camera.Camera) {
	screen.Clear()
